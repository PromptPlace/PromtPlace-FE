...
const LoginSwitchModal = ({ isOpen, onClose }: SocialLoginModalProps) => {
  const { switchAccount } = useAuth();
  if (!isOpen) return null;

  const handleSwitchGoogleLogin = () => {
    switchAccount();
    console.log('계정전환 전 로그아웃');
    // 1. 필요한 정보들을 정의합니다.
...
export default LoginSwitchModal;
