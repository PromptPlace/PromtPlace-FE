  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 bg-overlay-coach-mark z-[20]">
      <nav className="relative flex justify-between items-center gap-[2.6vw] py-[7.5px] pl-[61.25px] pr-[36px]">
        ...
        <div className="relative w-[127px] shrink-0">
          <PrimaryButton buttonType="tip" text="프롬프트 TIP" onClick={() => {}} />

          <div className="absolute top-[65px] right-[77px] flex gap-[14px]">
            <div className="text-white text-[18px] font-normal leading-[23px] mt-[21px] w-full whitespace-nowrap">
              프롬프트 작성 <span className="text-secondary-pressed">꿀팁</span>과
              <p className="text-secondary-pressed">공지사항</p>
            </div>

            <div className="w-[44.24px] h-[35.39px] shrink-0">
              <img src={ArrowTip} alt="arrow-tip" className="w-full h-full object-contain" />
            </div>
          </div>
        </div>
        ...
        {!accessToken && (
          <div className="relative w-[171px] shrink-0">
            ...
            <div className="absolute top-[65px] right-[85.6px] flex flex-col">
              <div className="flex justify-end w-[33px] h-[48px] shrink-0 w-full">
                <img src={ArrowLogin} alt="arrow-login w-full h-full object-contain" />
              </div>

              <div className="text-white text-[18px] font-normal leading-[23px] whitespace-nowrap">
                더 많은 서비스를
                <br />
        ...
        <div className="absolute top-[124px] left-1/2 -translate-x-1/2 w-[1171px] flex flex-col gap-[85px]">
          <div className="w-max bg-white rounded-[1000px] py-[10px] relative">
            <>
              <section className="h-11 px-5 inline-flex justify-center items-center gap-3.5 font-['Spoqa_Han_Sans_Neo'] text-text-on-background text-xl font-medium">
                {['모델', '필터', '태그'].map((label) => (
                  <div key={label} className="relative p-2.5 flex justify-start items-center gap-2.5 cursor-pointer">
                    <span>{label}</span>

                    <div className={`w-6 h-6 p-1.5 rounded-full flex justify-center items-center`}>
                ...
                <label className="ml-2 flex items-center gap-1 px-2 py-1 text-lg cursor-pointer">
                  <span>무료만 보기</span>
                  <input disabled type="checkbox" className="accent-primary-pressed w-4 h-4" />
                </label>
              </section>
            </>
          ...
          <div className="flex relative left-[130px] gap-[43px]">
