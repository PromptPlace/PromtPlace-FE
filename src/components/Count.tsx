...
const Count = ({ count, imgType }: CountProps) => {
  ...
  return (
    <div className="flex justify-center items-center gap-[10px] text-base font-normal leading-[20px] text-text-on-background max-lg:text-[8px] max-lg:leading-[10px]">
      <img src={imgSrc} alt={`${imgType} 이미지`} className="h-[16px] max-lg:h-[12px]" />
      <span>{count}</span>
    </div>
  );
};
