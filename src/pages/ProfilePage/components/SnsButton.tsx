...
const SnsButton = ({ url }: SnsButtonProps) => {
  ...
  return (
    <a href={url} target="_blank">
      <div className="flex py-[6px] px-[12px] gap-[8px] justify-center items-center rounded-[50px] border border-[0.8px] border-gray300 bg-white self-center">
        <img src={snsType} alt="sns" className="w-[24px] h-[24px]" />
        <p className="custom-body3 overflow-hidden text-ellipsis">@{`${url}`.slice(9, 20)}</p>
      </div>
    </a>
  );
};
