...
const PromptCreateTextPage = () => {
  ...
  const handleUploadPrompt = async () => {
    ...
    try {
      const res = await axiosInstance.post(`${base}/api/prompts`, {
        ...
        is_free: true, // 무료 고정
        price: 0, // 무료 고정
        model_version: modelver || '', // 모델 버전 추가
        categories: categories, // 카테고리 배열
        models: selectedModels, // 모델 배열
      });
      ...
      setModalText('업로드에 실패했어요. 다시 시도해주세요.');
      throw err;
    }
  };
  ...
  return (
    <>
      {/* 427px 이상*/}
      <div className="hidden min-[427px]:block">
...
글자 수 제한을 없애면 블로그/랜딩페이지용 문구에도 확장 가능`}
      ...
      {/* 426px 이하*/}
      <div className="block min-[427px]:hidden">B 화면 (426px 이하)</div>
    </>
  );
};
