        ...
        <div className="min-h-screen py-[64px]">
          ...
          <div className="flex flex-col items-start ml-[102px]">
            <div className="mb-[56px]">
              <h1 className="text-[32px] font-medium text-black mb-[12px]">공지사항</h1>
              <p className="text-[16px] font-light text-black">프롬프트 플레이스의 공지사항을 확인해보세요!</p>
            </div>
          </div>
              ...
              <div className="w-full">
                {notices.map((notice) => (
                  <div
                    ...
                    className="w-full bg-white py-[20px] px-[24px] flex gap-[20px] cursor-pointer"
                    onClick={() => {
                    ...
                    <div className="w-full flex flex-col justify-between">
                      <div>
                        <p className="text-[12px] text-gray-400 flex-shrink-0 font-light mb-[8px]">
                          {formatDate(notice.created_at)}
                        </p>

                        <div className="flex justify-between items-center">
                          <div className="flex items-center">
                            {new Date().getTime() - new Date(notice.created_at).getTime() <=
                              14 * 24 * 60 * 60 * 1000 && (
                              <img src={newBadgeImg} alt="NEW" className="w-[20px] h-[20px] mr-[12px]" />
                            )}
                            <p className="text-[14px] font-medium text-text-on-white">{notice.title}</p>
                          </div>
      ...
      <div className="block min-[481px]:hidden">
        <div className="min-h-screen px-[20px] py-[64px]">
          ...
          <div className="flex flex-col items-start">
            <div className="mb-[56px]">
              <h1 className="text-[24px] font-medium text-black mb-[12px]">공지사항</h1>
              <p className="text-[14px] font-light text-black">프롬프트 플레이스의 공지사항을 확인해보세요!</p>
            </div>
          </div>
            ...
            {!loading && notices.length > 0 && (
              <div className="w-full">
                {notices.map((notice) => (
                  <div
                    key={notice.announcement_id}
                    className="w-full bg-white py-[16px] px-[12px] flex gap-[20px] cursor-pointer"
                    onClick={() => {
                      handleRowClick(notice.announcement_id);
                    }}>
                    {/* 내용 */}
                    <div className="w-full flex flex-col justify-between">
                      <div>
                        <p className="text-[10px] text-gray-400 flex-shrink-0 font-light mb-[8px]">
                          {formatDate(notice.created_at)}
                        </p>
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center">
                            {new Date().getTime() - new Date(notice.created_at).getTime() <=
                              14 * 24 * 60 * 60 * 1000 && (
                              <img src={newBadgeImg} alt="NEW" className="w-[12px] h-[12px] mr-[12px]" />
                            )}
                            <p className="text-[12px] font-medium text-text-on-white">{notice.title}</p>
                          </div>
