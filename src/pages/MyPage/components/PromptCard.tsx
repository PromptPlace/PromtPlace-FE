  ...
  return (
    <div className="flex items-center   border-b-[1px] border-b-white-stroke w-full  py-[10px] h-[92px] bg-white">
      <Link to={`/prompt/${promptData.id}`} className="flex flex-row">
        <div className="flex items-center  text-text-on-white text-[22px] pl-[80px] font-bold w-[635px]">
          {promptData.title}
        </div>
        <div className="flex items-center justify-center text-text-on-background text-[20px] font-medium  w-[223px]">
          {promptData.model}
        </div>
      ...
      {type === 'downloaded' && (
        <div className="flex items-center justify-center text-text-on-white text-[20px] font-medium py-[23.5px] w-[180px] px-[44px]">
          {promptData.author}
        </div>
      )}
      ...
      {type === 'liked' && (
        <button onClick={() => DeleteLike(promptData.id)}>
          <img src={BigBlueHeart} alt="좋아요 큰하트" className=" h-[22px]  w-[115px]" />
        </button> // 하트 아이콘
      )}
    </div>
  );
