    ...
    <div className="flex min-w-0 flex-1 flex-col gap-[12px] rounded-[8px] bg-gray-50 px-[16px] py-[12px] min-h-[240px]">
      {items.map((item) => (
        <div key={item.month} className="flex items-center justify-between">
          <span className="custom-body1 text-text-on-white">{`${year}년 ${item.month}월`}</span>
          <span className="custom-h4 text-gray-700">{formatPrice(item.amount)}</span>
        </div>
      ))}
    </div>
  );
  ...
  if (hasNoSettlements) {
    return (
      <section className="flex flex-col gap-[20px]">
        <h2 className="custom-h2 text-text-on-white">{TOTAL_SETTLEMENT_SECTION_TITLE}</h2>

        <div className="flex flex-col gap-[20px] rounded-[12px] bg-white p-[24px]">
          <p className="custom-button1 text-gray-700">{TOTAL_SETTLEMENT_NOTICE_TEXT}</p>
    ...
    <section className="flex flex-col gap-[20px]">
      ...
      <div className="flex flex-col gap-[20px] rounded-[12px] bg-white p-[24px]">
        ...
        <div className="flex flex-col gap-[20px] lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h3 className="custom-h4 text-text-on-white">{TOTAL_SETTLEMENT_LABEL}</h3>
            <p className="custom-h1 text-primary">{formatPrice(data.totalAccumulatedAmount)}</p>
          </div>
