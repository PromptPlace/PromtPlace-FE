...
const UserVisitDashboard = () => {
  const { data: visitorData } = useGetVisitorStats();
  const { data: activeUserData } = useGetActiveUserStats();
  const visitorStats = visitorData?.data;
  const activeUserStats = activeUserData?.data;

  const visitorChange = formatChangeRate(visitorStats?.change_rate ?? null);
  const activeUserChange = formatChangeRate(activeUserStats?.change_rate ?? null);
      ...
      <div className="flex mt-4 justify-center items-center gap-[15px]">
        ...
        <div className="bg-background rounded-xl w-58 h-20 px-4 py-3 gap-2">
          ...
          <div className="flex mt-2 gap-2">
            <div className="text-lg text-gray-700">{(visitorStats?.current_count ?? 0).toLocaleString()}명</div>
            {visitorChange && (
              <div className="flex">
                <img
                  className="w-5 h-5"
        ...
        <div className="bg-background rounded-xl w-58 h-20 px-4 py-3 gap-2">
          ...
          <div className="flex mt-2 gap-2">
            <div className="text-lg text-gray-700">{(activeUserStats?.current_count ?? 0).toLocaleString()}명</div>
            {activeUserChange && (
              <div className="flex">
                <img
                  className="w-5 h-5"
